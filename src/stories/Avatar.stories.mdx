...
1. If there is no `src`, `Avatar` will attempt to get initials automatically based off the `name` prop (likely the user's name). A max of 3 initials will be displayed. Alternatively, you can override the default initials behavior and set your own initials manually using the `CustomInitials` prop.
1. If there is no `src`, `name`, or `CustomInitials` (i.e. a newly invited user who hasn't finished setting up their account), we'll display a fallback generic user icon.
...
## Colors
Avatars will have a default background color of `var(--cc_color_background_3)`, but you can override that with the `backgroundColor` prop which accepts levels 1, 2, and 3 background colors from our tokens color library. For example, you might be rendering Avatars inside a container that already has a level 3 background color, in which case you could change the `backgroundColor` prop to `var(--cc_color_background_1)`.

To represent a user from a collaborating company, use the `color` prop to create a color border around the Avatar. The `color` prop accepts `color_collab_[color]` token values.
...
## Tooltip Hover
Add the `showNameOnHover` prop if you want to show a tooltip with the Avatar's `name` inside. You can choose a `tooltipPosition` as well if the default (`top-center`) doesn't work for you.
...
<Canvas>
  <Story
    ...
    args={{
      ...
      style: {"margin": "32px -32px 32px 64px"}
    }}
  >
    {Template.bind({})}
  </Story>
